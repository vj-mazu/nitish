...
const Records: React.FC = () => {
  ...
  // Rice Stock filter state - Set default dates to show recent data
  ...
  }, [riceStockDateFrom, riceStockDateTo, riceStockProductType, riceStockLocationCode, riceStockPage]);
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      if (data.pagination?.totalRecords && data.pagination.totalRecords > 250) {
        const recordsShown = data.pagination.recordsReturned || Object.values(processedRecords).flat().length;
        const totalRecords = data.pagination.totalRecords;
        const currentPage = data.pagination.page || page;
        const totalPages = data.pagination.totalPages || 1;

        if (activeTab === 'stock' && selectedMonth) {
          const monthLabel = availableMonths.find(m => m.month === selectedMonth)?.month_label || 'Selected Month';
          toast.success(`📅 ${monthLabel}: Loaded ${recordsShown} paddy stock entries`);
        } else if (showAllRecords) {
          toast.success(`📊 Page ${currentPage}/${totalPages}: Showing ${recordsShown} of ${totalRecords} total records`);
        } else if (selectedMonth) {
          toast.success(`📅 Month view: Showing ${recordsShown} of ${totalRecords} records`);
        } else {
          toast.info(`Showing ${recordsShown} of ${totalRecords} records. Use pagination to view more.`);
        }
      }
  ...
  const fetchRiceStock = async () => {
    ...
    try {
      ...
      if (activeTab === 'rice-outturn-report' || activeTab === 'rice-stock') {
        ...
        // Fetch rice productions with date filters
        ...
        if (riceStockDateFrom) productionsParams.dateFrom = riceStockDateFrom;
        if (riceStockDateTo) productionsParams.dateTo = riceStockDateTo;
        ...
        try {
          ...
          // Add date filters if available
          if (riceStockDateFrom) movementsParams.dateFrom = riceStockDateFrom;
          if (riceStockDateTo) movementsParams.dateTo = riceStockDateTo;
        ...
        if (riceStockDateFrom || riceStockDateTo || riceStockProductType || riceStockLocationCode) {
          ...
          allMovements = allMovements.filter((movement: any) => {
            // Date filtering
            if (riceStockDateFrom || riceStockDateTo) {
              const movementDate = new Date(movement.date);
              if (riceStockDateFrom) {
                const fromDate = new Date(riceStockDateFrom);
                if (movementDate < fromDate) return false;
              }
              if (riceStockDateTo) {
                const toDate = new Date(riceStockDateTo);
        ...
        if (allMovements.length > 0) {
          toast.success(`Loaded ${allMovements.length} rice stock records`);
        } else {
          toast.info('No rice stock records found for the selected filters');
        }
      }
    } catch (error: any) {
  ...
  return (
    <Container>
      ...
      <FilterSection>
        ...
        <FilterRow>
          {/* Month Selector for Paddy Stock / Overview */}
          {(activeTab === 'stock' || activeTab === 'arrivals' || activeTab === 'purchase' || activeTab === 'shifting') && (
            <FormGroup>
          ...
          <FormGroup>
            ...
            <Input
              type="text"
              value={activeTab === 'rice-stock' || activeTab === 'rice-outturn-report' ? riceStockDateFrom : dateFrom}
              onChange={(e) => (activeTab === 'rice-stock' || activeTab === 'rice-outturn-report') ? setRiceStockDateFrom(e.target.value) : setDateFrom(e.target.value)}
          ...
          <FormGroup>
            ...
            <Input
              type="text"
              value={activeTab === 'rice-stock' || activeTab === 'rice-outturn-report' ? riceStockDateTo : dateTo}
              onChange={(e) => (activeTab === 'rice-stock' || activeTab === 'rice-outturn-report') ? setRiceStockDateTo(e.target.value) : setDateTo(e.target.value)}
              placeholder="DD-MM-YYYY"
            />
          </FormGroup>
