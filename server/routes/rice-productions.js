...
// Get all rice productions with pagination
router.get('/', auth, async (req, res) => {
  try {
    const { dateFrom, dateTo, outturnId, status, limit = 100, page = 1 } = req.query;

    console.log('🔍 Rice productions query params:', { dateFrom, dateTo, outturnId, status, limit, page });
    ...
    if (dateFrom || dateTo) {
      ...
      console.log('📅 Date filter applied:', where.date);
    }
    ...
    // Pagination setup
    const limitNum = Math.min(parseInt(limit), 500); // Max 500 per page for performance
    const pageNum = parseInt(page) || 1;
    const offset = (pageNum - 1) * limitNum;

    // Get total count for pagination
    const totalCount = await RiceProduction.count({ where });
    ...
    console.log(`✅ Found ${productions.length} rice productions (page ${pageNum}/${totalPages})`);
