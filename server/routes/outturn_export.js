...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (byProducts.length > 0) {
      ...
      // Adjusted for portrait A4 (535px available width)
      const colWidths = [65, 75, 65, 75, 65, 65, 65, 85]; // Total: 560px - slightly over but manageable
      const headers = ['Rice', 'Rejection Rice', 'Broken', 'Rejection Broken', 'Zero Broken', 'Faram', 'Bran', 'Date'];
      ...
      byProducts.forEach((bp, idx) => {
        ...
        values.forEach((val, i) => {
          const x = tableStartX + colWidths.slice(0, i).reduce((a,b) => a+b, 0);
          doc.text(val, x + 3, y + 5, { width: colWidths[i] - 6, align: i < 7 ? 'right' : 'center' });
        });
